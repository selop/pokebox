...
// Tera Shiny Rare configurable uniforms
...
void main() {
    ...
    // ── Classic TCG holo shine (mask-driven, 133° angle, mosaic) ──
    // Gate mosaic by foil threshold: mosaic only where foil > threshold
    float foilGate = smoothstep(uMosaicFoilThreshold - 0.05, uMosaicFoilThreshold + 0.05, foil);
    float gatedMosaicIntensity = uMosaicIntensity * foilGate;

    result = holoShineMosaic(
        result, uv, uPointer, uBackground.y, mask,
        uMaskThreshold, uRainbowScale, uRainbowShift,
        uHoloOpacity, uCardOpacity, 133.0,
        uMosaicScale, gatedMosaicIntensity,
        uMosaicSaturation, uMosaicContrast
    );
